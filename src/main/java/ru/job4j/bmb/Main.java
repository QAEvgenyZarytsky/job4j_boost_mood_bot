...
public class Main {
    ...
    CommandLineRunner loadDatabase(MoodRepository moodRepository,
        ...
        return args -> {
            ...
            saveMoodContents(moodRepository, moodContentRepository);
            saveAwards(awardRepository);
        };
    }

    private void saveMoodContents(MoodRepository moodRepo, MoodContentRepository moodContentRepo) {
        var moodsWithContents = List.of(
                ...
                new MoodContent(new Mood("Успокоение и гармония \uD83E\uDDD8\u200D♂\uFE0F", true),
                ...
                new MoodContent(new Mood("Тревожное настроение \uD83D\uDE1F", true),
                        "Не волнуйтесь, всё пройдет. Попробуйте расслабиться и найти источник вашего беспокойства."),
                new MoodContent(new Mood("Разочарованное настроение \uD83D\uDE1E", true),
                        "Бывает. Не позволяйте разочарованию сбить вас с толку, всё наладится."),
                new MoodContent(new Mood("Усталое настроение \uD83D\uDE34", true),
                ...
                new MoodContent(new Mood("Раздраженное настроение \uD83D\uDE20", true),
                        "Попробуйте успокоиться и найти причину раздражения, чтобы исправить ситуацию.")
        );

        var moods = moodsWithContents.stream()
                .map(MoodContent::getMood)
                .distinct()
                .toList();

        moodRepo.saveAll(moods);
        moodContentRepo.saveAll(moodsWithContents);
    }

    private void saveAwards(AwardRepository awardRepo) {
        var awards = List.of(
                ...
                new Award("Награда 'Настроение месяца'", "За поддержание хорошего или отличного настроения в течение целого месяца. " + "Награда: Специальный значок, признание в сообществе или дополнительные привилегии.", 31),
